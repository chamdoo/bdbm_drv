...
int AmfRequestmakeReq_cb (  struct PortalInternal *p, const AmfRequestT req, const uint32_t offset ) {
    (static_cast<AmfRequestWrapper *>(p->parent))->makeReq ( req, offset);
    return 0;
};
int AmfRequestdebugDumpReq_cb (  struct PortalInternal *p, const uint32_t card ) {
...
int AmfIndicationreadDone_cb (  struct PortalInternal *p, const uint32_t tag ) {
    (static_cast<AmfIndicationWrapper *>(p->parent))->readDone ( tag);
    return 0;
};
int AmfIndicationwriteDone_cb (  struct PortalInternal *p, const uint32_t tag ) {
    (static_cast<AmfIndicationWrapper *>(p->parent))->writeDone ( tag);
    return 0;
};
int AmfIndicationeraseDone_cb (  struct PortalInternal *p, const uint32_t tag, const uint32_t status ) {
