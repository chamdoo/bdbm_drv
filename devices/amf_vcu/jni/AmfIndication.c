...
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_AmfIndication_eraseDone, 3);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    ...
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_AmfIndication_eraseDone << 16) | 3, -1);
    return 0;
};
...
const uint32_t AmfIndication_reqinfo = 0x7001c;
const char * AmfIndication_methodSignatures()
{
    return "{\"respReadBlkInfo\": [\"long\"], \"readDone\": [\"long\"], \"eraseDone\": [\"long\", \"long\"], \"debugDumpResp\": [\"long\", \"long\", \"long\", \"long\", \"long\", \"long\"], \"respAftlFailed\": [\"long\"], \"writeDone\": [\"long\"], \"respReadMapping\": [\"long\", \"long\"]}";
}
